...
not to the databases Metabase reports against. The password comes from
`METABASE_PG_PASSWORD` or a prompt — never from a flag.
...
`--help`), so a project can pin its settings and run with no arguments.
